      ...
      >
        ...
        <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-slate-200 bg-slate-50 gap-2">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1 justify-center sm:justify-start">
            ...
            <h2 className="font-bold text-slate-900 truncate">Handbuch</h2>
          ...
          <button
            ...
            className="sm:hidden flex items-center gap-1.5 px-2.5 py-1.5 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg text-sm font-semibold text-slate-800 transition-colors shrink-0"
            aria-label="Inhaltsverzeichnis öffnen"
          >
            <Menu className="w-4 h-4" />
          </button>

          {/* Desktop: separate close button */}
          <button
            onClick={onClose}
            className="hidden sm:flex p-1 hover:bg-slate-200 rounded-lg transition-colors"
        ...
        <div className="flex flex-1 overflow-hidden relative">
          {/* Sidebar — always visible on sm+, slide-in drawer on mobile */}
          <aside
            className={
              "border-r border-slate-200 overflow-y-auto bg-slate-50 py-2 " +
              "sm:w-56 sm:shrink-0 sm:static sm:translate-x-0 " +
              "absolute inset-y-0 left-0 z-10 w-72 max-w-[85%] transition-transform shadow-xl sm:shadow-none " +
              (showSidebar ? "translate-x-0" : "-translate-x-full sm:translate-x-0")
            }
          >
            <div className="sm:hidden flex items-center justify-between px-3 py-2 border-b border-slate-200 mb-2">
              <span className="font-bold text-slate-900 text-sm">Inhalt</span>
              <button
            ...
            <nav className="text-sm">
              ...
              <div className="sm:hidden mt-4 px-3 pt-3 border-t border-slate-200 flex flex-col gap-2">
                <a
                  href="https://github.com/reifen01/Morgenroutine/blob/main/HANDBUCH.md"
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          {showSidebar && (
            <div
              className="sm:hidden absolute inset-0 bg-slate-900/40 z-[5]"
              onClick={() => setShowSidebar(false)}
            />
          )}
          ...
          <div
            ref={contentRef}
            className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 text-slate-800 text-[15px] sm:text-sm leading-relaxed"
          >
            <ReactMarkdown
